      ...
      <MvpToolbar
        ...
        filters={
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
            <select value={status} onChange={(event) => setStatus(event.target.value)} className="h-10 rounded-lg border border-[#102c3d]/[0.09] bg-white px-3 text-sm font-semibold text-[#102c3d]/[0.66]"><option>Active</option><option>Archived</option><option>All</option></select>
            <select value={departmentFilter} onChange={(event) => setDepartmentFilter(event.target.value)} className="h-10 rounded-lg border border-[#102c3d]/[0.09] bg-white px-3 text-sm font-semibold text-[#102c3d]/[0.66]"><option>All</option>{departments.map((department) => <option key={department}>{department}</option>)}</select>
            <details className="group rounded-lg border border-[#102c3d]/[0.09] bg-white">
              <summary className="flex h-10 cursor-pointer list-none items-center justify-between gap-2 px-3 text-sm font-semibold text-[#102c3d]/[0.66]">
                More filters
              ...
              <div className="grid gap-2 border-t border-[#102c3d]/[0.06] p-2">
                <select value={managerFilter} onChange={(event) => setManagerFilter(event.target.value)} className="h-10 rounded-lg border border-[#102c3d]/[0.09] bg-white px-3 text-sm font-semibold text-[#102c3d]/[0.66]"><option value="All">All managers</option>{managers.map((manager) => <option key={manager.id} value={manager.id}>{manager.name}</option>)}</select>
                <select value={siteFilter} onChange={(event) => setSiteFilter(event.target.value)} className="h-10 rounded-lg border border-[#102c3d]/[0.09] bg-white px-3 text-sm font-semibold text-[#102c3d]/[0.66]"><option value="All">All sites</option>{sites.map((site) => <option key={site}>{site}</option>)}</select>
                <select value={applicationFilter} onChange={(event) => setApplicationFilter(event.target.value)} className="h-10 rounded-lg border border-[#102c3d]/[0.09] bg-white px-3 text-sm font-semibold text-[#102c3d]/[0.66]"><option value="All">All applications</option>{applicationStatuses.map((statusOption) => <option key={statusOption}>{statusOption}</option>)}</select>
              </div>
            </details>
          </div>
        }
      ...
      {visible.length ? (
        <div className="grid gap-4 xl:grid-cols-2">
          {visible.map((employee) => {
            ...
            return (
              <article key={employee.id} className="border border-[#102c3d]/[0.07] bg-white p-4 shadow-[0_10px_24px_rgba(16,44,61,0.03)]">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <p className="truncate text-base font-semibold text-[#102c3d]">{employee.name}</p>
                    <p className="mt-1 text-sm leading-6 text-[#102c3d]/[0.58]">{employee.jobTitle || role?.title || "Role to confirm"}</p>
                  </div>
                ...
                <div className="mt-3 grid gap-x-5 gap-y-2 border-t border-[#102c3d]/[0.06] pt-3 sm:grid-cols-2">
                ...
                {routeOutcome && !operationalSummary?.hasLearnerLifecycle ? (
                  <div className="mt-3 border-l-2 border-[#4f7b95] bg-[#f8fbfa] px-4 py-3">
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-[#102c3d]/[0.06] pt-4">
                  {isLineManager ? (
                    ...
                    <>
                      ...
                      <button type="button" onClick={() => setSelectedEmployeeId(employee.id)} className="rounded-full bg-[#f5f7f3] px-4 py-2 text-xs font-semibold text-[#102c3d]/[0.62] ring-1 ring-[#102c3d]/[0.07] transition hover:text-[#102c3d]">
                        Details
                      </button>
                    </>
                  )}
        ...
        canWriteEmployees ? (
          ...
          <div className="rounded-xl border border-dashed border-[#102c3d]/[0.14] bg-[#f8fbfa] px-5 py-10 text-center">
            ...
            <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#102c3d]/[0.56]">Your view is limited to your own profile and direct reports. Ask a workspace admin if your reporting line looks incorrect.</p>
          </div>
        )
      )}
      ...
      {draft ? (
        <MvpModal title={data.employees.some((item) => item.id === draft.id) ? "Edit employee" : "Add employee"} onClose={() => setDraft(null)}>
          <form onSubmit={submit}>
            <div className="mb-5 flex items-start gap-3 rounded-xl bg-[#edf7f3] p-4 ring-1 ring-[#159b8f]/[0.10]">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-[#102c3d]/[0.54]">After saving, LevyTate Copilot can guide the role and capability conversation before any application begins.</p>
              </div>
            </div>
            ...
            <details className="group mt-5 rounded-xl border border-[#102c3d]/[0.07] bg-[#f8fbfa]">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 text-xs font-semibold text-[#102c3d]/[0.58]">
      ...
      {selectedEmployee ? (
        <MvpModal title={selectedEmployee.name} eyebrow="Employee apprenticeship record" onClose={() => setSelectedEmployeeId(null)} wide>
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-[#f8fbfa] px-4 py-3 ring-1 ring-[#102c3d]/[0.055]">
            <p className="text-sm leading-6 text-[#102c3d]/[0.58]">Keep the record simple. Use AI for discovery, then open details only when needed.</p>
          ...
          <div className="grid gap-5 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
            <div className="space-y-4 rounded-xl border border-[#102c3d]/[0.07] bg-[#f8fbfa] p-4">
              <section>
                ...
                <p className="mt-2 text-sm leading-6 text-[#102c3d]/[0.56]">{selectedEmployee.department || "Department to confirm"} - {selectedEmployee.site || "Site to confirm"}</p>
                <p className="mt-2 text-sm leading-6 text-[#102c3d]/[0.56]">Manager: {managerName(data, selectedEmployee)}</p>
              </section>
              ...
              <section>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {employeeDevelopmentInterests(data, selectedEmployee.id).length ? employeeDevelopmentInterests(data, selectedEmployee.id).map((interest) => (
                    <span key={interest} className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-[#102c3d]/[0.68] ring-1 ring-[#102c3d]/[0.08]">{interest}</span>
                  )) : <p className="text-sm text-[#102c3d]/[0.54]">No interests captured yet.</p>}
                </div>
              </section>
              ...
              <section>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-[#102c3d]/[0.48]">Sourced from the employee conversation and role library.</p>
                  </div>
                  ...
                  <div className="mt-2 rounded-xl bg-white px-4 py-3 ring-1 ring-[#102c3d]/[0.07]">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-xs leading-5 text-[#102c3d]/[0.56]">{selectedRouteOutcome.copy}</p>
                      </div>
                      <StatusBadge tone={selectedRouteOutcome.tone}>{selectedRouteOutcome.badge}</StatusBadge>
                    </div>
                  </div>
                ) : <p className="mt-2 text-sm text-[#102c3d]/[0.54]">LevyTate will show the leading recommendation once discovery is complete.</p>}
              </section>
            </div>
              ...
              <section className="rounded-xl border border-[#102c3d]/[0.07] bg-white p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-[#102c3d]/[0.54]">Every workflow step is kept on the employee record.</p>
                  </div>
                  <TableAction onClick={() => onStartDiscovery?.(selectedEmployee.id)}>Open Copilot</TableAction>
                </div>
                    ...
                    <div key={application.id} className="rounded-xl bg-[#f8fbfa] px-4 py-3 ring-1 ring-[#102c3d]/[0.06]">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-xs leading-5 text-[#102c3d]/[0.54]">{application.reason || "No reason recorded yet."}</p>
                        </div>
                        <StatusBadge tone={statusTone(application.status)}>{application.status}</StatusBadge>
                      </div>
                      <div className="mt-3 space-y-2">
                        {application.history.map((entry) => (
                          <div key={entry.id} className="rounded-lg bg-white px-3 py-2 text-xs text-[#102c3d]/[0.62] ring-1 ring-[#102c3d]/[0.05]">
                            <div className="flex items-center justify-between gap-3">
                              <p className="font-semibold text-[#102c3d]">{entry.status}</p>
                              <span className="text-[#102c3d]/[0.42]">{entry.owner}</span>
                            </div>
                            <p className="mt-1">{entry.note}</p>
                            <p className="mt-1 text-[#102c3d]/[0.42]">{entry.createdAt.slice(0, 10)}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  )) : <p className="text-sm text-[#102c3d]/[0.54]">No applications created yet for this employee.</p>}
                </div>
              </section>
              ...
              <section className="rounded-xl border border-[#102c3d]/[0.07] bg-white p-4">
                ...
                <div className="mt-4 grid gap-3">
                  {selectedProfile?.conversationHistory.length ? selectedProfile.conversationHistory.slice(-6).map((message, index) => (
                    <div key={`${message.role}-${index}`} className={`rounded-xl px-4 py-3 text-sm leading-6 ${message.role === "user" ? "bg-[#102c3d] text-white" : "bg-[#f8fbfa] text-[#102c3d]/[0.68] ring-1 ring-[#102c3d]/[0.06]"}`}>
                      {message.content}
                    </div>
                  )) : <p className="text-sm text-[#102c3d]/[0.54]">Conversation history will appear after guided discovery begins.</p>}
                </div>
              </section>
            </div>
          </div>
...
function DecisionFact({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="mt-1 line-clamp-2 text-sm font-semibold leading-5 text-[#102c3d]/[0.72]">{value}</p>
    </div>
  );
}
