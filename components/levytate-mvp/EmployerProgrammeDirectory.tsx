  ...
  return (
    <div className="grid gap-6">
      <header>
        ...
        <h1 className="mt-2 text-3xl font-semibold tracking-[-0.025em]">Programmes &amp; Providers</h1>
        <p className="mt-2 text-sm leading-6 text-[#102c3d]/60">Explore apprenticeship programmes available through LevyTate&apos;s provider directory.</p>
      </header>

      <section aria-label="Directory summary" className="grid grid-cols-2 gap-3 sm:max-w-md">
        <Count label="Programmes" value={results.length} />
        <Count label="Providers" value={visibleProviders} />
      </section>

      <section aria-label="Programme directory filters" className="grid gap-3 rounded-[1.4rem] border border-[#102c3d]/[0.07] bg-white p-4 sm:p-5">
        <label className="relative block">
          <Search className="absolute left-4 top-3.5 text-[#102c3d]/40" size={18} aria-hidden="true" />
          <span className="sr-only">Search programmes and providers</span>
          <input value={filters.search} onChange={(event) => updateFilter("search", event.target.value)} placeholder="Search programmes, providers, standards, roles or skills" className="min-h-12 w-full rounded-xl border border-[#102c3d]/10 bg-[#fbfcfb] pl-11 pr-4 text-sm outline-none transition focus:border-[#159b8f] focus:ring-4 focus:ring-[#159b8f]/10" />
        </label>
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <Filter label="Delivery" value={filters.delivery} values={options.delivery} onChange={(value) => updateFilter("delivery", value)} />
          <Filter label="Location" value={filters.location} values={options.locations} onChange={(value) => updateFilter("location", value)} />
          <Filter label="Category" value={filters.category} values={options.categories} onChange={(value) => updateFilter("category", value)} />
        </div>
...
function Count({ label, value }: { label: string; value: number }) {
  return <div className="rounded-2xl bg-[#edf7f3] p-4"><p className="text-2xl font-semibold text-[#102c3d]">{value}</p><p className="mt-1 text-xs font-semibold uppercase tracking-[0.12em] text-[#0b6f63]">{label}</p></div>;
}

function Filter({ label, value, values, onChange }: { label: string; value: string; values: string[]; onChange: (value: string) => void }) {
  return <label className="grid gap-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#102c3d]/45">{label}<select aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} className="min-h-11 min-w-0 rounded-xl border border-[#102c3d]/10 bg-white px-3 text-sm font-semibold normal-case tracking-normal text-[#102c3d]"><option>All</option>{values.map((item) => <option key={item}>{item}</option>)}</select></label>;
}
...
function ProgrammeCard({ item, compared, onOpen, onCompare }: { item: DirectoryProgramme; compared: boolean; onOpen: () => void; onCompare: () => void }) {
  return (
    <article className="flex min-h-[330px] flex-col rounded-[1.4rem] border border-[#102c3d]/[0.08] bg-white p-5 shadow-[0_14px_34px_rgba(16,44,61,0.045)]">
      <div className="flex items-start justify-between gap-3"><span className="rounded-full bg-[#edf7f3] px-3 py-1 text-xs font-semibold text-[#0b6f63]">{levelLabel(item)}</span><span className="text-right text-[10px] font-semibold uppercase tracking-[0.1em] text-[#102c3d]/45">{verificationLabel(item.programme, item.provider)}</span></div>
      ...
      <p className="mt-3 line-clamp-3 text-sm leading-6 text-[#102c3d]/60">{clean(item.programme.shortDescription) || missingInformation}</p>
      <dl className="mt-4 grid gap-2 text-xs text-[#102c3d]/58"><Fact icon={GraduationCap} value={standardLabel(item)} /><Fact icon={Timer} value={durationLabel(item.programme)} /><Fact icon={Building2} value={deliveryLabel(item.programme, item.provider)} /><Fact icon={MapPin} value={coverageLabel(item.programme, item.provider)} /></dl>
      <div className="mt-auto grid grid-cols-[1fr_auto] gap-2 pt-5"><button type="button" onClick={onOpen} className="min-h-11 rounded-full bg-[#102c3d] px-4 text-sm font-semibold text-white hover:bg-[#17394d] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#159b8f]">View programme</button><button type="button" aria-label={`${compared ? "Remove" : "Add"} ${item.programme.programmeName} ${compared ? "from" : "to"} comparison`} onClick={onCompare} className={`grid min-h-11 min-w-11 place-items-center rounded-full ring-1 ${compared ? "bg-[#edf7f3] text-[#0b6f63] ring-[#159b8f]/20" : "bg-white text-[#102c3d]/60 ring-[#102c3d]/10"}`}><GitCompareArrows size={17} /></button></div>
    </article>
  );
}
...
function ProviderProfile({ provider, programmes, onBack, onProgramme }: { provider: ProviderCatalogueRecord; programmes: DirectoryProgramme[]; onBack: () => void; onProgramme: (id: string) => void }) {
  ...
  return (
    <div className="mx-auto grid max-w-5xl gap-5">
      <Back onClick={onBack}>Back to Programmes &amp; Providers</Back>
      <header className="rounded-[1.6rem] bg-[#102c3d] p-6 text-white sm:p-8"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#7ad1c5]">Provider profile</p><h1 className="mt-3 text-3xl font-semibold tracking-[-0.03em]">{provider.providerName}</h1><p className="mt-4 max-w-3xl text-sm leading-7 text-white/72">{overview}</p><p className="mt-4 text-xs text-white/60">Provider information is presented to support employer research and comparison.</p></header>
...
function Badge({ children }: { children: string }) {
  return <span className="rounded-full bg-white/70 px-3 py-1 text-xs font-semibold text-[#0b6f63]">{children}</span>;
}

function DetailFact({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl border border-[#102c3d]/[0.07] bg-white p-4"><dt className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#102c3d]/45">{label}</dt><dd className="mt-2 text-sm font-semibold leading-6">{value}</dd></div>;
}

function Copy({ title, value }: { title: string; value: string }) {
  return <section className="rounded-[1.4rem] border border-[#102c3d]/[0.07] bg-white p-5"><h2 className="text-lg font-semibold">{title}</h2><p className="mt-3 text-sm leading-7 text-[#102c3d]/62">{value}</p></section>;
}

function Info({ title, items }: { title: string; items: string[] }) {
  return <section className="rounded-[1.4rem] border border-[#102c3d]/[0.07] bg-white p-5"><h2 className="text-lg font-semibold">{title}</h2>{items.length ? <ul className="mt-3 grid gap-2">{items.map((item) => <li key={item} className="flex gap-2 text-sm leading-6 text-[#102c3d]/62"><Check size={15} className="mt-1 shrink-0 text-[#0b8e82]" />{item}</li>)}</ul> : <p className="mt-3 text-sm leading-6 text-[#102c3d]/55">{missingInformation}</p>}</section>;
}

function CompareFact({ label, value }: { label: string; value: string }) {
  return <div><dt className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#102c3d]/42">{label}</dt><dd className="mt-0.5 leading-5 text-[#102c3d]/70">{value}</dd></div>;
}
