...
export function LevyTateLoginClient({ internalLoginEnabled, invalidLink = false, providerLogoutWarning = false }: { internalLoginEnabled: boolean; invalidLink?: boolean; providerLogoutWarning?: boolean }) {
  ...
  return <main className="grid min-h-screen place-items-center overflow-x-hidden bg-[#f6fbf8] px-5 py-10 text-[#102c3d]">
    <section className="w-full max-w-md rounded-[1.5rem] border border-[#102c3d]/[0.08] bg-white p-6 shadow-[0_30px_90px_rgba(16,44,61,0.1)]">
      ...
      {providerLogoutWarning ? <p className="mt-5 rounded-2xl bg-[#fff8e8] px-4 py-3 text-sm text-[#7a5818]">We signed you out locally, but there was a problem completing server sign-out.</p> : null}
      ...
      <nav aria-label="Trust and support" className="mt-6 flex flex-wrap justify-center gap-x-4 gap-y-2 border-t border-[#102c3d]/[0.08] pt-5 text-xs font-semibold text-[#102c3d]/55">
        ...
        <Link className="min-h-11 content-center hover:text-[#087c73] focus:outline-none focus:ring-2 focus:ring-[#159b8f]/30" href="/levytate/early-access-terms">Terms</Link>
