...
const roleContent: Record<AssistantRole, { purpose: string; welcome: string; prompts: string[] }> = {
  Employee: {
    ...
    welcome: "Tell me about your role, what is changing and where you would like to progress. I will help you work out the next useful question before we settle on a pathway.",
    prompts: [
      "I'm in an admin role but I'm starting to do more reporting and automation. What should I do?",
      "What apprenticeship could help me move into data work?",
      "Help me prepare a conversation with my manager.",
      "Can you compare two pathways for me?",
    ],
  },
  "Line Manager": {
        ...
        <div className="flex min-h-0 flex-col">
          <div className="flex items-center justify-between gap-4 border-b border-[#102c3d]/[0.07] px-5 py-4">
            <div>
              ...
              <p className="mt-1 text-xs text-[#102c3d]/48">{role} mode</p>
            </div>
            ...
            {messages.map((message) => (
              <article key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[92%] rounded-2xl px-4 py-3 text-sm leading-6 shadow-[0_8px_20px_rgba(16,44,61,0.04)] sm:max-w-[82%] ${message.role === "user" ? "bg-[#102c3d] text-white" : "bg-white text-[#102c3d]/72 ring-1 ring-[#102c3d]/[0.06]"}`}>
                  ...
                  {message.response ? <InlineResponse response={message.response} onAction={chooseAction} onQuickReply={(reply) => void sendMessage(reply)} loading={loading} /> : null}
                </div>
              </article>
            ))}
            {loading ? (
          ...
          <form onSubmit={submit} className="border-t border-[#102c3d]/[0.07] bg-white p-4">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
              <textarea
                ...
                rows={2}
                placeholder="Describe the role, goal or decision you are working through"
          ...
          <div className="mt-5 border-t border-[#102c3d]/[0.07] pt-5">
            ...
            <div className="mt-3 grid gap-2">
              {roleContent[role].prompts.map((prompt) => (
                <button key={prompt} type="button" onClick={() => void sendMessage(prompt)} disabled={loading} className="rounded-xl bg-[#f8fbfa] px-3.5 py-3 text-left text-xs font-semibold leading-5 text-[#102c3d]/66 ring-1 ring-[#102c3d]/[0.055] transition hover:bg-white hover:text-[#102c3d] hover:ring-[#159b8f]/20 disabled:opacity-50">
                  {prompt}
                </button>
              ))}
            </div>
