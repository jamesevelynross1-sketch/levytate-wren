...
export function RolesModule() {
  ...
  const [alternativePathways, setAlternativePathways] = useState("");
  ...
  function open(role?: MvpRole) {
    ...
    setDraft(next);
    setPrimaryPathway(next.pathwayMappings.find((item) => item.recommendationType === "Primary")?.pathwayTitle ?? "");
    setAlternativePathways(next.pathwayMappings.filter((item) => item.recommendationType === "Alternative").map((item) => item.pathwayTitle).join(", "));
  ...
  function submit(event: FormEvent) {
    ...
    if (!draft.title.trim() || !draft.department.trim() || !primaryPathway.trim()) { setError("Role title, department and primary pathway are required."); return; }
    const alternatives = splitMvpList(alternativePathways);
    const mappings = [
      { id: createMvpId("mapping"), pathwayTitle: primaryPathway.trim(), recommendationType: "Primary" as const, priority: 1, businessRationale: rationale.trim() || "Primary role-led pathway mapping.", fundingRoute: "Potentially funded through levy/co-investment" as const, deliveryPreference: "Blended" as const },
      ...alternatives.map((title, index) => ({ id: createMvpId("mapping"), pathwayTitle: title, recommendationType: "Alternative" as const, priority: index + 2, businessRationale: "Alternative role-led pathway.", fundingRoute: "Potentially funded through levy/co-investment" as const, deliveryPreference: "Blended" as const })),
    ];
    saveRole({ ...draft, title: draft.title.trim(), department: draft.department.trim(), pathwayMappings: mappings, updatedAt: nowIso() });
    setDraft(null);
  }

  return <MvpPanel title="Role library" eyebrow="Pathway source of truth"><MvpToolbar search={search} onSearch={setSearch} placeholder="Search roles, departments or skills" actionLabel="Add role" onAction={() => open()} filters={<select value={status} onChange={(event) => setStatus(event.target.value)} className="h-10 rounded-lg border border-[#102c3d]/[0.09] bg-white px-3 text-sm font-semibold"><option>Active</option><option>Archived</option><option>All</option></select>} />{visible.length ? <TableShell><TableHead><tr><th className="px-4 py-3">Role</th><th className="px-4 py-3">Department</th><th className="px-4 py-3">Career level</th><th className="px-4 py-3">Primary pathway</th><th className="px-4 py-3">Linked employees</th><th className="px-4 py-3 text-right">Actions</th></tr></TableHead><TableBody>{visible.map((role) => <tr key={role.id}><td className="px-4 py-3"><p className="font-semibold">{role.title}</p><p className="mt-0.5 text-xs text-[#102c3d]/48">{role.businessArea || "Business area not set"}</p></td><td className="px-4 py-3 text-[#102c3d]/62">{role.department}</td><td className="px-4 py-3"><StatusBadge>{role.careerLevel}</StatusBadge></td><td className="px-4 py-3 text-[#102c3d]/62">{role.pathwayMappings.find((item) => item.recommendationType === "Primary")?.pathwayTitle}</td><td className="px-4 py-3 text-[#102c3d]/62">{data.employees.filter((employee) => employee.roleId === role.id).length}</td><td className="px-4 py-3"><div className="flex justify-end gap-2"><TableAction onClick={() => open(role)}>Edit</TableAction><TableAction onClick={() => archiveRole(role.id)} danger={role.status === "Active"}>{role.status === "Archived" ? "Restore" : "Archive"}</TableAction></div></td></tr>)}</TableBody></TableShell> : <EmptyState title="No roles yet" copy="Add a role and map its primary and alternative apprenticeship pathways." actionLabel="Add role" onAction={() => open()} />}{draft ? <MvpModal title={data.roles.some((item) => item.id === draft.id) ? "Edit role" : "Add role"} onClose={() => setDraft(null)} wide><form onSubmit={submit}><FormGrid><FormField label="Role title" value={draft.title} onChange={(value) => setDraft({ ...draft, title: value })} required /><FormField label="Department" value={draft.department} onChange={(value) => setDraft({ ...draft, department: value })} required /><FormField label="Business area" value={draft.businessArea} onChange={(value) => setDraft({ ...draft, businessArea: value })} /><FormSelect label="Career level" value={draft.careerLevel} onChange={(value) => setDraft({ ...draft, careerLevel: value as MvpRole["careerLevel"] })} options={careerLevels} /><FormField label="Skills tags" value={draft.skillsTags.join(", ")} onChange={(value) => setDraft({ ...draft, skillsTags: splitMvpList(value) })} wide /><FormField label="Typical progression" value={draft.progression.join(", ")} onChange={(value) => setDraft({ ...draft, progression: splitMvpList(value) })} wide /><FormField label="Primary pathway" value={primaryPathway} onChange={setPrimaryPathway} required wide /><FormField label="Alternative pathways" value={alternativePathways} onChange={setAlternativePathways} placeholder="Comma separated" wide /><FormTextArea label="Business rationale" value={rationale} onChange={setRationale} wide /></FormGrid><FormActions onCancel={() => setDraft(null)} label="Save role" error={error} /></form></MvpModal> : null}</MvpPanel>;
}