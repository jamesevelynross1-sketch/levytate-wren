...
export function LevyFinanceModule({ organisationId, demoMode }: { organisationId: string; demoMode: boolean }) {
  ...
  return <div className="grid gap-5" data-testid="levy-finance-module">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div className="flex items-center gap-2"><PoundSterling size={18} className="text-[#0b776e]" /><p className="text-sm font-semibold text-[#102c3d]">Levy position</p></div>
      <div className="text-right"><p className="text-xs font-semibold text-[#0b776e]">{state.mode === "fixture" ? "Illustrative finance data" : "Latest DAS import"}</p><p className="mt-1 text-xs text-[#102c3d]/[0.44]">{lastImport ? `Imported ${displayDate(lastImport.importedAt)}` : "No data imported"}</p></div>
    </div>
    ...
    <section className="grid border-y border-[#102c3d]/[0.08] bg-white sm:grid-cols-2 xl:grid-cols-5">
      ...
      <Metric label="Levy received this month" value={formatGbp(latest?.levyReceivedPence ?? 0)} copy="" />
      <Metric label="Levy spend this month" value={formatGbp(latest?.apprenticeshipSpendPence ?? 0)} copy="" />
      ...
      <Metric label="Expired this month" value={formatGbp(latest?.expiredPence ?? 0)} copy="" tone="coral" />
    </section>

    {summary.currentBalance.source === "unavailable" ? <section className="flex flex-wrap items-end gap-3 border border-[#e6b74a]/[0.25] bg-[#fffaf0] p-4"><div className="min-w-[220px] flex-1"><p className="font-semibold">Confirm current DAS balance</p><p className="mt-1 text-xs text-[#102c3d]/[0.52]">Enter the balance currently shown in your Apprenticeship Service account.</p></div><input aria-label="Current DAS balance" value={balanceInput} onChange={(event) => setBalanceInput(event.target.value)} placeholder="£0" className="h-11 w-40 border border-[#102c3d]/[0.10] bg-white px-3 text-sm" /><button onClick={confirmBalance} className="h-11 bg-[#102c3d] px-4 text-sm font-semibold text-white">Confirm balance</button></section> : null}

    <section className="border border-[#102c3d]/[0.07] bg-white p-4 sm:p-5">
      <div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-lg font-semibold">Levy in vs apprenticeship spend</h2><p className="mt-1 text-xs text-[#102c3d]/[0.48]">Monthly values from the imported DAS transaction history.</p></div><div className="flex gap-1">{([6,12,24,"all"] as const).filter((item) => item === "all" || summary.monthly.length >= item).map((item) => <button key={item} onClick={() => setRange(item)} className={`min-h-9 px-3 text-xs font-semibold ${range === item ? "bg-[#102c3d] text-white" : "bg-[#f4f7f5] text-[#102c3d]/[0.58]"}`}>{item === "all" ? "All" : `${item}M`}</button>)}</div></div>
...
function FinanceChart({ months }: { months: ReturnType<typeof calculateLevyFinance>["monthly"] }) {
  ...
  return <div className="mt-5"><div className="flex h-52 items-end gap-2 overflow-hidden border-b border-[#102c3d]/[0.10]" role="img" aria-label="Monthly levy received compared with apprenticeship spend">{months.map((month) => <div key={month.month} className="flex h-full min-w-0 flex-1 items-end justify-center gap-1" title={`${monthLabel(month.month)}: ${formatGbp(month.levyReceivedPence)} received, ${formatGbp(month.apprenticeshipSpendPence)} spent`}><span className="w-2 max-w-[18px] flex-1 bg-[#159b8f]" style={{height:`${Math.max(3,month.levyReceivedPence/max*100)}%`}} /><span className="w-2 max-w-[18px] flex-1 bg-[#102c3d]" style={{height:`${Math.max(3,month.apprenticeshipSpendPence/max*100)}%`}} /></div>)}</div><div className="mt-2 flex justify-between text-[10px] text-[#102c3d]/[0.42]"><span>{months[0] ? monthLabel(months[0].month) : "No data"}</span><span>{months.at(-1) ? monthLabel(months.at(-1)?.month ?? "") : ""}</span></div><div className="mt-3 flex gap-4 text-xs"><span className="flex items-center gap-2"><i className="h-2 w-2 bg-[#159b8f]" />Levy received</span><span className="flex items-center gap-2"><i className="h-2 w-2 bg-[#102c3d]" />Apprenticeship spend</span></div></div>;
}

function MonthlyTable({ months }: { months: ReturnType<typeof calculateLevyFinance>["monthly"] }) { return <div className="mt-4 grid gap-2"><div className="hidden grid-cols-6 gap-3 border-b border-[#102c3d]/[0.08] pb-2 text-[10px] font-semibold uppercase tracking-[0.08em] text-[#102c3d]/[0.38] md:grid"><span>Month</span><span className="text-right">Received</span><span className="text-right">Spend</span><span className="text-right">Difference</span><span className="text-right">Expired</span><span className="text-right">Balance</span></div>{[...months].reverse().map((month) => <div key={month.month} className="grid gap-2 border-b border-[#102c3d]/[0.06] py-3 text-sm md:grid-cols-6"><strong>{monthLabel(month.month)}</strong><Cell label="Received" value={formatGbp(month.levyReceivedPence)} /><Cell label="Spend" value={formatGbp(month.apprenticeshipSpendPence)} /><Cell label="Difference" value={formatGbp(month.spendVsContributionsPence)} /><Cell label="Expired" value={formatGbp(month.expiredPence)} /><Cell label="Balance" value={month.closingBalancePence === undefined ? "Not reported" : formatGbp(month.closingBalancePence)} /></div>)}</div>; }
function Cell({ label, value }: { label: string; value: string }) { return <span className="flex justify-between gap-3 tabular-nums md:block md:text-right"><small className="text-[#102c3d]/[0.42] md:hidden">{label}</small>{value}</span>; }
function AuditRows({ record, balanceSource }: { record?: LevyFinanceState["imports"][number]; balanceSource: string }) { if (!record) return <p className="mt-3 text-sm text-[#102c3d]/[0.48]">No import recorded.</p>; const rows = [["File",record.fileName],["Rows",String(record.sourceRows)],["New",String(record.newTransactions)],["Duplicates ignored",String(record.duplicateRows)],["Needs review",String(record.reviewRows)],["Balance source",balanceSource]]; return <dl className="mt-3 grid gap-2">{rows.map(([label,value]) => <div key={label} className="flex justify-between gap-3 text-xs"><dt className="text-[#102c3d]/[0.44]">{label}</dt><dd className="max-w-[190px] truncate text-right font-semibold">{value}</dd></div>)}</dl>; }
