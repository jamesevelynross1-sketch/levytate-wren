...
export function PersistentCopilot({ context, initialEmployeeId, onNavigate }: { context: LevyTateCopilotContext; initialEmployeeId?: string | null; onNavigate?: (target: string) => void }) {
  ...
  return <>
    <button ref={launcherRef} type="button" aria-label="Open Copilot" aria-expanded={open} aria-controls="levytate-persistent-copilot" onClick={() => setOpen(true)} className={`fixed bottom-6 right-0 z-40 hidden min-h-11 items-center justify-center gap-2 rounded-l-lg border border-r-0 border-[#102c3d]/[0.15] bg-[#102c3d] px-3 py-3 text-xs font-semibold text-white shadow-[0_8px_20px_rgba(16,44,61,.12)] transition duration-150 hover:bg-[#17394d] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#159b8f] focus-visible:ring-offset-2 lg:flex ${open ? "pointer-events-none translate-x-full opacity-0" : ""}`}><PanelRightOpen size={16} aria-hidden="true" />Copilot</button>
