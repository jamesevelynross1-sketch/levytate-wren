...
export function buildLevyTateAiSystemPrompt(request: LevyTateAiRequest) {
  ...
  return [
    ...
    "Your job is to feel like a warm career and apprenticeship support assistant.",
    "Answer the user's actual question first, then explain boundaries or next steps.",
    ...
    return JSON.stringify(
      {
        ...
        deterministicGuidance: {
          ...
          availableNow: fallback.employeeGuidance?.availableNow ?? [],
          futureInterests: fallback.employeeGuidance?.futureInterests ?? [],
          intent: fallback.employeeGuidance?.intent ?? "general_support",
        ...
        instructions: [
          ...
          "Use casual, supportive, human language. Avoid rigid words like primary recommendation or strongest next fit unless the user specifically asks for a recommendation.",
          "Do not start with the active application warning. Start by answering the user's question.",
          ...
          "For Daniel asking about management, compare data leadership with broader people or operational management.",
