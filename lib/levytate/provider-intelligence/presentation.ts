import type { IntelligencePresentation, PresentedIntelligenceUpdate, ProviderIntelligenceArticle } from "./domain.ts";
...
export function assignFeedPresentation(feed: readonly ProviderIntelligenceArticle[]): PresentedIntelligenceUpdate[] {
  const prominentProviders = new Set<string>();
  return feed.map((item, index) => {
    let presentation = item.contentType === "Event" ? "event" : rhythm[index % rhythm.length];
