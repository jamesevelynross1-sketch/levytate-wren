...
export function buildLevyTateAiSystemPrompt(request: LevyTateAiRequest) {
  return [
    ...
    "If the relevant guidance is not present in trustedGuidance.approvedSources, use the trustedGuidance.fallbackMessage exactly and do not infer the rule.",
  ...
  return JSON.stringify(
    {
      ...
      responseRules: [
        ...
        "For policy, funding, employer responsibility, learner responsibility, provider selection and standards governance questions, rely only on groundedContext.trustedGuidance.approvedSources. If no suitable source is present, use groundedContext.trustedGuidance.fallbackMessage exactly.",
