...
check("canonical employer label", policy.includes('item("Marketplace", "Marketplace", "enabled", "discover", "core")'));
check("Platform Admin label remains distinct", policy.includes('item("Providers", "Provider Catalogue", "enabled", "manage", "core")'));
...
check("employer surface excludes ranking and recommendation scores", !/match percentage|match score|star rating|fit score|relevance score|ranking number|AI confidence|recommendation score|provider leaderboard|matching engine|weighted relevance|algorithm score|recommendation percentage|internal fit/i.test(directory));
...
check("Copilot deep links open Marketplace profiles", copilot.includes("module=Marketplace&programme=") && copilot.includes("module=Marketplace&provider="));
