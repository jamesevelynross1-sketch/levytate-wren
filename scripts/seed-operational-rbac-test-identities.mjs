...
function lifecycleScenarios() {
  return [
    ...
    },
    {
      ...
      managerCheckIn: { date: "2026-07-07", next: "2026-08-07", summary: "Morgan confirmed Ben has protected time and access to a suitable reporting project.", actions: ["Review the next Power BI milestone"], support: "No additional manager support required.", status: "completed" },
      eligibilityConfirmed: true,
    ...
    },
    {
      ...
      managerCheckIn: { date: "2026-05-30", next: "2026-06-30", summary: "Morgan identified that Cara needs a live reporting project and clearer evidence checkpoints.", actions: ["Agree a recovery project", "Confirm weekly evidence time"], support: "Manager intervention is overdue.", status: "action_required" },
      eligibilityConfirmed: true,
      eligibilityStatus: "employer_verified",
      workingHours: 88,
      probationStatus: "passed",
    ...
    },
    {
      ...
      managerCheckIn: { date: "2026-06-18", next: "2026-08-01", summary: "Morgan agreed to confirm Daniel's rota capacity before return.", actions: ["Confirm return plan with the Apprenticeship Lead"], support: "A return-to-learning plan is required.", status: "scheduled" },
      eligibilityConfirmed: true,
      eligibilityStatus: "employer_verified",
      workingHours: 95,
      probationStatus: "passed",
    ...
    },
    {
      ...
      managerCheckIn: { date: "2026-07-06", next: "2026-08-06", summary: "Morgan reviewed Finley's workplace evidence and agreed the remaining sign-off plan.", actions: ["Confirm final workplace evidence"], support: "Manager confirmation remains part of assessment readiness.", status: "completed" },
      eligibilityConfirmed: true,
      eligibilityStatus: "employer_verified",
      workingHours: 100,
      probationStatus: "passed",
    ...
    },
    {
      ...
      managerCheckIn: { date: "2026-07-01", next: "2026-08-01", summary: "Morgan confirmed Grace has the workplace support needed during assessment.", actions: ["Keep Grace informed of the result window"], support: "No immediate manager intervention.", status: "completed" },
      eligibilityConfirmed: true,
      eligibilityStatus: "employer_verified",
      workingHours: 93,
      probationStatus: "passed",
...
function buildLifecycleRows(organisationId, scenarios) {
  ...
  for (const scenario of scenarios) {
    ...
    if (scenario.managerCheckIn) {
      reviews.push(reviewRow(organisationId, recordId, scenario.managerCheckIn, "manager_check_in", ""));
    }
    ...
    if (scenario.assessment) {
      assessments.push({
        ...
        expected_assessment_start_date: scenario.key === "assessment-prep" ? "2026-09-15" : scenario.assessment.actual || null,
        assessment_start_date: scenario.assessment.status === "in_assessment" ? scenario.assessment.actual || null : null,
        assessment_status: scenario.assessment.status,
        assessment_organisation: "Provider assessment partner",
        assessment_contact: "Assessment coordination team",
        assessment_reference: `GC-ASSESS-${scenario.key.toUpperCase()}`,
        assessment_notes: scenario.assessment.notes,
        confirmations: Object.fromEntries(["provider", "learner", "line_manager", "employer"].map((type, index) => [type, {
          type,
          status: scenario.key === "assessment-prep" && type !== "provider" ? "awaiting_confirmation" : "confirmed",
          confirmedDate: scenario.key === "assessment-prep" && type !== "provider" ? "" : `2026-06-${String(14 + index).padStart(2, "0")}`,
          confirmedBy: scenario.key === "assessment-prep" && type !== "provider" ? "" : type === "provider" ? "Provider reviewer" : type === "learner" ? "Learner confirmation" : type === "line_manager" ? "Line manager" : "Priya Shah",
          recordedOnBehalfOf: "",
          note: "",
          evidenceReference: type === "provider" ? `GC-PROVIDER-${scenario.key.toUpperCase()}` : "",
          updatedBy: "apprenticeshiplead.demo@levytate.test",
          updatedAt: now,
        }])),
        readiness_confirmed_by: scenario.assessment.status === "preparing" ? "" : "apprenticeshiplead.demo@levytate.test",
        readiness_confirmed_at: scenario.assessment.status === "preparing" ? null : now,
        updated_by: "apprenticeshiplead.demo@levytate.test",
        version: 1,
        created_at: now,
        updated_at: now,
      });
...
function reviewRow(organisationId, learnerRecordId, review, type, providerId) {
  return {
    ...
    reviewer_name: type === "provider_review" ? "Provider Skills Coach" : type === "manager_check_in" ? "Morgan Price" : "Priya Shah",
    reviewer_user_id: type === "provider_review" ? "" : type === "manager_check_in" ? "gc-rbac-employee-morgan" : "gc-rbac-employee-priya",
    provider_id: providerId,
