...
export default function PortakabinApprenticeshipHub() {
  ...
  const filteredRequests = useMemo(() => filterBySite(requests, selectedSite, allSitesLabel), [requests, selectedSite]);
  const filteredLearners = useMemo(() => filterBySite(portakabinLearners, selectedSite, allSitesLabel), [selectedSite]);
...
function ApprenticeshipLeadAIPage({ selectedSite }: { selectedSite: string }) {
  ...
  const [matchingRequests, setMatchingRequests] = useState<ProviderMatchingRequest[]>(initialProviderMatchingRequests);
